...
func Setup() {
	...
	pflag.StringVar(&webhookCertDir, "webhookCertDir", "/tmp/k8s-webhook-server/webhook-certs", "Path to the webhook server certificate folder")
	...
	pflag.StringVar(&authCertDir, "authCertDir", "/tmp/k8s-webhook-server/auth-certs", "Path to the auth server certificate folder")
